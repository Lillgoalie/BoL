...
function OnLoad()
	...
	PrintChat("<font color = \"#33CCCC\">Orianna by</font> <font color = \"#fff8e7\">Honda, Hellsing & Lillgoalie</font>")
end
...
function CastW()
	...
	if hitcount >= 1 and VIP_USER then
		Packet('S_CAST', {spellId=_W}):send()
	end
	if hitcount >= 1 and not VIP_USER then
		CastSpell(_W)
	end
end
...
function CastR(target)
	...
	if GetDistance(position, BallPos) < Rradius and GetDistance(target, BallPos) < Rradius and VIP_USER then
		Packet('S_CAST', {spellId = _R}):send()
	end
	if GetDistance(position, BallPos) < Rradius and GetDistance(target, BallPos) < Rradius and not VIP_USER then
		CastSpell(_R)
	end
end
...
function Farm(Mode)
	...
	if UseW and WREADY then
		...
		if Hit >= 3 and VIP_USER then
			Packet('S_CAST', {spellId=_W}):send()
		end

		if Hit >= 3 and not VIP_USER then
			CastSpell(_W)
		end
	end
...
function FarmJungle()
	...
	if Minion then
		...
		if UseW and WREADY and GetDistance(BallPos, Minion) < Wradius and VIP_USER then
			Packet('S_CAST', {spellId=_W}):send()
		end

		if UseW and WREADY and GetDistance(BallPos, Minion) < Wradius and not VIP_USER then
			CastSpell(_W)
		end
